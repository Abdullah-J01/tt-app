...
/**
 * Modern plum-gradient footer as a rounded floating card, with slow drifting +
 * fading background blobs (animate-blob, disabled under prefers-reduced-motion).
 */
export function Footer() {
  return (
    <footer className="px-4 pb-8 pt-16 sm:px-6">
      <div className="shadow-lift relative mx-auto max-w-7xl overflow-hidden rounded-3xl bg-gradient-to-br from-violet via-violet-dark to-plum-1 text-white">
        {/* Accent bar */}
        ...
        <ParallaxBlobs>
          <span className="absolute -left-16 top-8 h-56 w-56 rounded-full bg-white/40 blur-3xl" />
          <span className="absolute bottom-0 left-1/2 h-64 w-72 rounded-full bg-amber/40 blur-3xl" />
          <span className="absolute -right-16 top-1/3 h-56 w-56 rounded-full bg-brand-green/40 blur-3xl" />
        </ParallaxBlobs>
        ...
        <div className="relative z-10 px-6 sm:px-10">
          ...
          <div className="flex flex-col items-start gap-4 py-10 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Link
              href="/feed"
              className="group inline-flex h-12 items-center gap-2 rounded-full bg-white px-6 font-semibold text-violet transition-transform hover:-translate-y-0.5 active:scale-[0.98]"
            >
              Open the app
          ...
          {/* Bottom bar */}
          <div className="flex flex-col gap-2 border-t border-white/10 py-4 text-xs text-white/60 sm:flex-row sm:justify-between">
            <span>
